...
{
	assert(param);
	CRtspTcpStreamReceiver* receiver = (CRtspTcpStreamReceiver*)param;
	receiver->RtspWorker();
	return 0;
}

CRtspTcpStreamReceiver::CRtspTcpStreamReceiver(const char* url, StreamDataCallBack func, void* userParam)
	: m_rtspUrl(url)
	, m_func(func)
	, m_user(userParam)
{
...
CRtspTcpStreamReceiver::~CRtspTcpStreamReceiver()
{
	Stop();
}

void CRtspTcpStreamReceiver::DeleteThis()
{
	delete this;
}

int CRtspTcpStreamReceiver::Start(int streamType)
{
...
int CRtspTcpStreamReceiver::Stop()
{
	m_running = false;
...
void CRtspTcpStreamReceiver::RtspWorker()
{
...
int CRtspTcpStreamReceiver::InitRtspSession_()
{
	if (m_rtspUrl.empty())
		return -1;
...
void CRtspTcpStreamReceiver::UnInitRtspSession_()
{
	if (m_command.get())
	{
...
int CRtspTcpStreamReceiver::ParseUrl_(const std::string& url, std::string& username, std::string& userpasswd, std::string& ip, int& port)
{
